...
fn main() {
    let private_key_str = valida_rs::io::read_and_deserialize::<String>().unwrap();
    let private_key = serde_json::from_str::<Scalar>(&private_key_str).unwrap();

    let amount = valida_rs::io::read_and_deserialize::<u64>().unwrap();

    let g_str = valida_rs::io::read_and_deserialize::<String>().unwrap();
    let g = serde_json::from_str::<AffinePoint>(&g_str).unwrap();

    let h_str = valida_rs::io::read_and_deserialize::<String>().unwrap();
